...
| **3** | **設定は不要** — `.obsidian` 設定を同梱しており、**Bases は既にオン**です。既にこのフォルダで Obsidian が独自の設定を作っていた場合は **設定 → コアプラグイン** で Bases がオンか確認してください |
...
```
.obsidian/   設定済み：Bases オン、新しい添付は images/ へ
...
データベース表示になります（Bases は設定済みで、手動でオンにする必要はありません）。
画像・リンク・グラフも動きます。
