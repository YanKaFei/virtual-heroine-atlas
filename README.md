...
**As a vault.** Open the folder in Obsidian. `views/library.base` gives a table of
all 132 entries grouped by language, by platform, by status; `views/factors.base`
gives one view per factor, so *who was carried by F04* is a single click. Bases is
a core plugin, nothing to install.
...
- **Not a source of art.** Character designs belong to the streamers and their
  artists. This atlas describes them; it does not distribute them, and there are no
  images in this repository on purpose.
