...
| **3** | **不用配置** —— 仓库自带 `.obsidian` 配置，**Bases 已经开好了**。若你的 Obsidian 此前已为这个文件夹生成过自己的配置，去 **设置 → 核心插件** 确认 Bases 是开的 |
...
```
.obsidian/   预配置：Bases 已开启，新附件落到 images/
...
按上面三步装好后，`views/` 里的 Base 文件就是现成的数据库视图（Bases 已预置开启，不用手动开）。
图像、双链、关系图谱都直接可用。
